...
- **Loại chiến lược:** Article chunking (tùy chỉnh, cắt theo heading/section)
- **Mô tả & lý do chọn:** Tài liệu chính sách có tiêu đề mục rõ ràng, nên cắt tại ranh giới heading cấp 2 giữ trọn mỗi mục làm một chunk, nội dung không tách khỏi tiêu đề mục. Tiêu đề cấp 1 của tài liệu được gắn làm tiền tố cho mọi chunk để chunk mang ngữ cảnh chủ đề khi embed. Mục nào dài quá ngưỡng thì chia tiếp bằng RecursiveChunker nhưng vẫn gắn lại tiền tố. Nếu tài liệu không có heading, chunker tự động lùi về RecursiveChunker. FixedSizeChunker(180, 20) được giữ làm đường cơ sở đối chứng.
- **Code snippet (nếu custom):**

```python
from src.chunking import ArticleChunker
chunker = ArticleChunker(max_chunk_size=900)
```
...
| Nguyễn Anh Đức   | Article (custom, theo heading) | 8.0                  | Giữ trọn mỗi mục, truy vết được nguồn tới tên mục | Chunk lớn hơn nên tín hiệu bị pha loãng khi một mục chứa nhiều ý rời nhau |
